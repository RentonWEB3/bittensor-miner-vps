    ...
    async def _start(self):
        ...
        while self.is_running:
            ...
            for scraper_id in scraper_ids_to_scrape_now:
                bt.logging.info(f"Creating scraper for {scraper_id}")
                try:
                    scraper = self.provider.get(scraper_id, self.scraping_config, self.session)
                    bt.logging.success(f"Successfully created scraper for {scraper_id}: {type(scraper)}")
                except Exception as e:
                    bt.logging.error(f"Failed to create scraper for {scraper_id}: {str(e)}")
                    bt.logging.error(f"Exception details: {e.__class__.__name__}: {str(e)}")
                    continue
