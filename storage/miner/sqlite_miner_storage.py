    ...
    def should_upload_hf_data(self, unique_id: str) -> bool:
        ...
        try:
            with contextlib.closing(self._create_connection()) as connection:
                ...
                # ТЕСТИРОВАНИЕ: Убираем блокировку - всегда разрешаем загрузку
                # time_difference = dt.timedelta(seconds=61200)  # 17 hours (отключено)
                # threshold_datetime = current_datetime - time_difference
                # return threshold_datetime > average_datetime

                return True  # Всегда разрешаем загрузку для тестирования
